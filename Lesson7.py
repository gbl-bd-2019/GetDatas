...
for i in regions:
    print(nom1, i)
    nom1 = nom1 + 1
sel_reg = int(input('Âûáåğèòå ğåãèîí (ââåäèòå ÷èñëî): ')) - 1

nom1 = 1
for i in names:
    print(nom1, i)
    nom1 = nom1 + 1
sel_name = int(input('Âûáåğèòå èíòåğåñóşùèå äàííûå (ââåäèòå ÷èñëî): ')) - 1
...
# d_min = '01.01.2016'
print(f"Âîçìîæíûé äèàïàçîí äàò: {dates[0].strftime('%d.%m.%y')} - {dates[-1].strftime('%d.%m.%y')}")
sel_dates = input('ââåäèòå èíòåğåñóşùèé äèàïàçîí â ôîğìàòå ÄÄ.ÌÌ.ÃÃ - ÄÄ.ÌÌ.ÃÃ: ')
d_min = sel_dates[:8]
d_max = sel_dates[11:]
